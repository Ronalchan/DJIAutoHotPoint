...
public class ConnectionActivity extends Activity implements View.OnClickListener {
     ...
     */
    private void checkAndRequestPermissions() {            //检查是否存在任何缺少的权限，并在需要时请求运行时权限
                    ...
                    DJISDKManager.getInstance().registerApp(getApplicationContext(), new DJISDKManager.SDKManagerCallback() {
                        ...
                        public void onRegister(DJIError djiError) {
                            if (djiError == DJISDKError.REGISTRATION_SUCCESS) {
                                DJILog.e("登录失败", DJISDKError.REGISTRATION_SUCCESS.getDescription());
                                DJISDKManager.getInstance().startConnectionToProduct();
                                showToast("登录成功");
                            } else {
                                showToast( "登录SDK失败，请检查网络");
                            }
    ...
    private void refreshSDKRelativeUI() {
        ...
        if (null != mProduct && mProduct.isConnected()) {
            ...
            if (null != mProduct.getModel()) {
                mTextProduct.setText("产品型号：" + mProduct.getModel().getDisplayName());
            } else {
