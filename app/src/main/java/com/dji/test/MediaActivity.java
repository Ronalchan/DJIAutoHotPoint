...
public class MediaActivity extends Activity implements View.OnClickListener {
    ...
    private Button mPlayBtn, mPauseBtn, mStopBtn, mMoveToBtn;
    ...
    protected void onDestroy() {
        ...
        DemoApplication.getCameraInstance().setMode(SettingsDefinitions.CameraMode.SHOOT_PHOTO, new CommonCallbacks.CompletionCallback() {
            ...
            public void onResult(DJIError mError) {
                if (mError != null){
                    setResultToToast("设置拍照模式失败" + mError.getDescription());
                }
            }
        });
    ...
    void initUI() {
        ...
        LinearLayoutManager layoutManager = new LinearLayoutManager(MediaActivity.this, OrientationHelper.VERTICAL,false);
        ...
        //Init Loading Dialog
        mLoadingDialog = new ProgressDialog(MediaActivity.this);
        mLoadingDialog.setMessage("请稍候");
        mLoadingDialog.setCanceledOnTouchOutside(false);
        mLoadingDialog.setCancelable(false);

        //Init Download Dialog
        mDownloadDialog = new ProgressDialog(MediaActivity.this);
        mDownloadDialog.setTitle("文件下载中...");
    ...
    private void setResultToToast(final String result) {
        runOnUiThread(new Runnable() {
            public void run() {
                Toast.makeText(MediaActivity.this, result, Toast.LENGTH_SHORT).show();
            }
        });
    }
                ...
                if (null != mMediaManager) {
                    ...
                    DemoApplication.getCameraInstance().setMode(SettingsDefinitions.CameraMode.MEDIA_DOWNLOAD, new CommonCallbacks.CompletionCallback() {
                        ...
                        public void onResult(DJIError error) {
                            if (error == null) {
                                DJILog.e(TAG, "设置模式：拍摄");
                                showProgressDialog();
                                getFileList();
                            } else {
                                setResultToToast("拍摄模式设置失败");
                            }
                        }
                    });
                    if (mMediaManager.isVideoPlaybackSupported()) {
                        DJILog.e(TAG, "摄像头支持录像回放！");
                    } else {
                        setResultToToast("摄像头不支持录像回放!");
                    }
                ...
                mMediaManager.refreshFileList(new CommonCallbacks.CompletionCallback() {
                    ...
                    public void onResult(DJIError error) {
                        if (null == error) {
                            ...
                            hideProgressDialog();
                            setResultToToast("获取媒体文件信息失败" + error.getDescription());
                        }
                    }
                });
            }
    ...
    private FetchMediaTask.Callback taskCallback = new FetchMediaTask.Callback() {
        ...
        public void onUpdate(MediaFile file, FetchMediaTaskContent option, DJIError error) {
            if (null == error) {
                ...
                DJILog.e(TAG, "获取任务失败" + error.getDescription());
            }
        }
    };
    ...
    private void updateStatusTextView(MediaManager.VideoPlaybackState videoPlaybackState) {
        ...
        if (videoPlaybackState != null) {
            if (videoPlaybackState.getPlayingMediaFile() != null) {
                addLineToSB(pushInfo, "文件相关信息", videoPlaybackState.getPlayingMediaFile().getIndex());
                addLineToSB(pushInfo, "文件大小", videoPlaybackState.getPlayingMediaFile().getFileSize());
                addLineToSB(pushInfo,
                        "持续时间",
                        videoPlaybackState.getPlayingMediaFile().getDurationInSeconds());
                addLineToSB(pushInfo, "创建时间", videoPlaybackState.getPlayingMediaFile().getDateCreated());
                addLineToSB(pushInfo,
                        "拍摄方向",
                        videoPlaybackState.getPlayingMediaFile().getVideoOrientation());
            } else {
                addLineToSB(pushInfo, "文件相关信息", "无");
            }
            ...
            addLineToSB(pushInfo, "已缓存：", videoPlaybackState.getCachedPercentage());
            addLineToSB(pushInfo, "存储位置", videoPlaybackState.getCachedPosition());
            pushInfo.append("\n");
    ...
    private void downloadFileByIndex(final int index){
        ...
        mediaFileList.get(index).fetchFileData(destDir, null, new DownloadListener<String>() {
            ...
            public void onFailure(DJIError error) {
                ...
                setResultToToast("下载失败：" + error.getDescription());
                currentProgress = -1;
            }
            ...
            public void onSuccess(String filePath) {
                HideDownloadProgressDialog();
                setResultToToast("下载成功" + ":" + filePath);
                currentProgress = -1;
            }
        });
    }
            ...
            public void onClick(DialogInterface dialog, int id) {
                ...
                mMediaManager.moveToPosition(Integer.parseInt(ms),
                        new CommonCallbacks.CompletionCallback() {
                            ...
                            public void onResult(DJIError error) {
                                if (null != error) {
                                    setResultToToast("跳转失败：" + error.getDescription());
                                } else {
                                    DJILog.e(TAG, "跳转成功");
                                }
                            }
        ...
        switch (v.getId()) {
            ...
            case R.id.stop_btn: {
                mMediaManager.stop(new CommonCallbacks.CompletionCallback() {
                    ...
                    public void onResult(DJIError error) {
                        if (null != error) {
                            setResultToToast("停止失败：" + error.getDescription());
                        } else {
                            DJILog.e(TAG, "成功停止");
                        }
                    }
    ...
    private void PauseandResume()
    {
        if(isPause == false){
            ...
            mMediaManager.resume(new CommonCallbacks.CompletionCallback() {
                ...
                public void onResult(DJIError error) {
                    if (null != error) {
                        setResultToToast("恢复失败" + error.getDescription());
                    } else {
                        DJILog.e(TAG, "成功恢复");
                    }
                }
